...
- [x] V2-T068 [REQ-045,REQ-049,REQ-053–059,REQ-061,REQ-062] Crear `database/setup.sql` P0 con 11 tablas, schemas, objetos, seeds, singleton y permisos en transacción; **Dep.** V2-T046,V2-T067; **Criterio** crea `[inovait_runtime]` sin login e incluye GRANT SELECT y DENY INSERT/UPDATE/DELETE exactos sobre `catalog.DocumentType`, equivalentes a migración; gate S12A si se activa el fallback.
...
- [x] V2-T070 [REQ-061] Producir una sola vez `IT-INDEXES-P0` al completar las pruebas de key/include/filter y FK support del modelo de 11 tablas en `RelationalIndexTests.cs`; **Dep.** V2-T068; **Criterio** ausencia de `Id` en INCLUDE para índices nonclustered, disponibilidad implícita por PK clustered, alerta documental si cambia clustering y gate S12B si se activa el fallback.
...
- [x] V2-T072 Crear `compose.yaml` para API/evaluador local con `mcr.microsoft.com/mssql/server:2022-CU14-ubuntu-22.04`, `ACCEPT_EULA=Y`, `MSSQL_SA_PASSWORD` externo a Git, puerto, volumen nombrado y health check; actualizar `README.md`, `quickstart.md` y `docs/evaluator-execution.md` con comandos validados y fallback `ConnectionStrings__InovaitDatabase`; **Dep.** V2-T071.
...
- [x] V2-T074 [US1–US3] [REQ-001–027,REQ-048,REQ-049] [SCN-001–019] Ejecutar y registrar evidencia fechada de los tres walkthroughs P0 contra el entorno disponible: inscripción con alta/reutilización/conflicto, consulta por contexto académico y contratación/consulta multiescuela; consumir la evidencia previa de paridad/runner e incluir órdenes deterministas y tiempos calentados informativos; **Dep.** V2-T073; **Estado** PASS: entorno local levantado con `compose.yaml`, `database/setup.sql` aplicado y ejecutados walkthroughs sobre API real con conexión fallback en 2 pasadas de ejecución (calentada + medición), incluyendo alta/reutilización/conflicto, contexto académico y multiescuela; evidencia con filtros y comandos conservadora.
- [x] V2-T075 [REQ-027,REQ-063] Ejecutar el gate humano obligatorio de S12 y emitir puerta P0 y forecast real por slice; **Dep.** V2-T074; **Criterio** ≤400 o activar S12A/S12B/S12C; solo un PASS autoriza V2-T076 y no hay excepción aprobada. **Estado** PASS: S12C aprobado, fallback aplicado por diseño (`S12C`) porque la suma humana de `S12A`/`S12B` excedía 400, `SLICE_BASE=4cf41d0`, `HUMAN_BASE=4cf41d0`, `HUMAN_HEAD` confirmado en este commit, `git diff --numstat ... | ./scripts/check-human-lines.py` = 329, `run-p0-tests.sh` PASS (37/37 IDs + 151 tests), y contrato/fixture P0 con 33 escenarios de walkthrough ejecutados verde (con evidencia temporal).
